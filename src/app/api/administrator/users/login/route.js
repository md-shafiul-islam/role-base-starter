...
export async function POST(request, ctx) {
  ...
  let loginResp = { response: null, status: true, message: "User login failed" };

  try {
    const login = await request.json();

    const { response, status, message } = await userController.getLoginUser(login);
    esBackLogger.info("User loggin ", status)
    ...
    return getResponseFormatter(loginResp.response, loginResp.status, loginResp.message);
  }
}
